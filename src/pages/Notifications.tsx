  ...
  const fetchNotifications = async () => {
    try {
      ...
      orders?.forEach(order => {
        ...
        if (diffDays < 0) {
          ...
          newNotifications.push({
            id: `overdue_${order.id}_${Date.now()}`,
            type: 'overdue',
            title: 'Order Overdue',
            message: `Order ${order.order_number} for ${order.customers?.name} is ${Math.abs(diffDays)} day${Math.abs(diffDays) !== 1 ? 's' : ''} overdue`,
            orderId: order.id,
          ...
          const priority = diffDays <= 1 ? 'high' : diffDays <= 3 ? 'medium' : 'low';
          newNotifications.push({
            id: `due_soon_${order.id}_${Date.now()}`,
      ...
      newNotifications
        ...
        .slice(0, 3) // Limit to 3 urgent notifications
