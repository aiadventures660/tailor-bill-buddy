...
const NewCustomer = () => {
  ...
  const handleSaveRecord = async () => {
    ...
    try {
      ...
      // Create a mapping function for garment names to clothing_type enum
      const mapGarmentToClothingType = (garmentName: string): 'shirt' | 'pant' | 'kurta_pajama' | 'suit' | 'blouse' | 'saree_blouse' | 'non_denim_pant' | 'short_kurta' | 'coat' | 'bandi' | 'westcot' | 'pajama' => {
        const lowerName = garmentName.toLowerCase();
        if (lowerName.includes('non denim pant') || lowerName.includes('non-denim pant')) {
          return 'non_denim_pant';
        } else if (lowerName.includes('short kurta')) {
          return 'short_kurta';
        } else if (lowerName.includes('coat') && !lowerName.includes('suit')) {
          return 'coat';
        } else if (lowerName.includes('bandi')) {
          return 'bandi';
        } else if (lowerName.includes('westcot') || lowerName.includes('waistcoat')) {
          return 'westcot';
        } else if (lowerName.includes('pajama') && !lowerName.includes('kurta')) {
          return 'pajama';
        } else if (lowerName.includes('pant') || lowerName.includes('trouser')) {
          return 'pant';
        } else if (lowerName.includes('kurta') || lowerName.includes('pajama')) {
          return 'kurta_pajama';
        } else if (lowerName.includes('suit') || lowerName.includes('coat')) {
          return 'suit';
        } else if (lowerName.includes('blouse') && lowerName.includes('saree')) {
          return 'saree_blouse';
        } else if (lowerName.includes('blouse')) {
          return 'blouse';
        } else {
          return 'shirt'; // Default for shirt, t-shirt, etc.
        }
      };
      ...
      const orderItems = selectedGarments.map(garment => ({
        ...
        total_price: 0,
        clothing_type: mapGarmentToClothingType(garment.name) as any, // Temporary type assertion until DB is updated
      }));
