...
const Payments: React.FC = () => {
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    ...
    try {
      // Atomic payment and order update via RPC
      const { data, error } = await supabase.rpc('add_payment_and_update_order', {
      ...
      if (error || (data && !data[0].success)) {
        throw new Error(data && data[0].error ? data[0].error : error?.message || 'Unknown error');
      }
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                        <Calendar className="h-4 w-4" />
                        <span>
                          Paid on {(payment.payment_date || payment.created_at)
